...
function Users() {
    ...
    useEffect(() => {
        setDisplayData(users.filter(ele => {
            return ele.email.includes(search) || ele.userid.includes(search) || ele.wallet.includes(search);
        }));
    }, [ users, search ]);

    let displayusers = displayData.slice((page - 1) * 10, page * 10);
